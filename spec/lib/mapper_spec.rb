...
describe Mapper do
  def default_config
    @default_config ||= Config.from(VALID_ENV)
  end
  ...
  it 'maps bat_fuel_charge' do
    ...
    expect(hash).to eq([{ field: 'battery_soc', measurement: 'PV', value: 81 }])
  end
  ...
  it 'maps case_temp' do
    ...
    expect(hash).to eq([{ field: 'case_temp', measurement: 'PV', value: 35 }])
  end

  it 'maps power_ratio' do
    hash = mapper.records_for('senec/0/PV1/POWER_RATIO', '70')

    expect(hash).to eq([{ field: 'grid_export_limit', measurement: 'PV', value: 70 }])
  end
  ...
  it 'handles unknown value types' do
    expect do
      mapper.records_for('senec/0/ENERGY/GUI_INVERTER_POWER', {})
    end.to raise_error(RuntimeError)
  end
end
